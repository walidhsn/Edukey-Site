<?php
	...
	//*********************************************************** 
	$all = $formationC->afficher_formation();
	if(isset($_GET['nom_categorie']))
	{
		$url = '?nom_categorie='.$_GET['nom_categorie'];
		header('Location:search_list.php'.$url);
	}
	/*if(isset($_POST['search']))
	{
		if(isset($_POST['category-id'])){
			if(!empty($_POST['category-id'])){
				$url = '?nom_categorie='.$_POST['category-id'];
				header('Location:search_list.php'.$url);
			}
		}
	}*/
					...
					<main>
						...
						<div class="category-search">
						  ...
						  <form action="search_list.php" method="post"> 
							 ...
							 <button type="submit" class="cws-button smaller border-radius alt" name="search">Search</button>
							</form>
						</div>
						<!-- / search -->
						<!-- item -->
						<?php foreach($all as $row){ 
						  	$img=$formationC->display_img($row['id_formation']); ?>
						<div class="category-item list clear-fix">
							<div class="picture">
								<div class="hover-effect"></div>
								<div class="link-cont">
									<a href="display_course.php?id_formation=<?=$row['id_formation']?>" class="fancy fa fa-search"></a>
								</div>
								<img src="<?=$img['img']?>" data-at2x="<?=$img['img']?>" alt>
							</div>
							<h3>Course of <?=$row['nom'] ?></h3>
							<div>
								<div class="star-rating" title="Rated 4.00 out of 5">
									<span style="width:<?=($row['taux']*20)?>%"></span>
								</div>
							</div>
							<p><?=$row['description']?> </br> <?=$row['categorie']?> </p>
