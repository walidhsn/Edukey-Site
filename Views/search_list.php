<?php
    ...
    $row = $formationC->chercher_par_categorie($nom_categorie);
    $count = sizeof($row);
    if(isset($_POST['keyword']))
    {
        $keyword = $_POST['keyword'];
        $row = $formationC->chercher_par_editor($keyword);
        $count = sizeof($row);
        if($count == 0)
        {
            $row = $formationC->chercher_par_prix($keyword);
            $count = sizeof($row);
            if($count == 0)
            {
                $row = $formationC->chercher_par_categorie($keyword);
                $count = sizeof($row);
                if($count == 0)
                {
                    $row = $formationC->chercher_par_nom($keyword);
                    $count = sizeof($row);
                    if($count == 0)
                    {
                        $row = $formationC->chercher_glob($keyword);
                        $count = sizeof($row);
                    }
                }
            }
...
<html>
<head>
	<title>Edukey - COURSES LIST </title>
	<meta charset="utf-8">
							...
							<li class="megamenu">
								...
								<?php foreach($result_categorie as $tab){?>
									<li><div class="header-megamenu"><?=$tab['nom_categorie']?></div>
										<ul>
											<li><a href="courses-list.php?id_categorie=<?=$tab['id_categorie']?>">Courses</a></li>
										</ul>
									</li>
									<?php }?>
        ...
        <div class="page-title">
			<div class="grid-row">
				...
				<nav class="bread-crumb">
					...
					<a href="courses-list.php">Courses - List</a>
				</nav>
			</div>
		</div>
	</header>
					...
					<main>
						...
						<div class="category-search">
                            ...
                            <select name="category-id" onchange="location = this.value;">
									...
									<?php 
									$result_categorie = $categorieC->afficher_categorie();
										foreach($result_categorie as $tab2) : ?>
									<option value="search_list.php?nom_categorie=<?=$tab2['nom_categorie']?>"><?=$tab2['nom_categorie']?></option>
									<?php endforeach; ?>									
								</select><!-- 
						 --> <form action="" method="post"> <!-- 
							 --><!-- 
							 --><input type="text" class="input-text" value placeholder="Keywords" name="keyword"><!-- 
							 --><button class="cws-button smaller border-radius alt" name="search">Search</button>
							</form>
						</div>
						<!-- / search -->
						<!-- item -->
						<?php for($i=0;$i<$count;$i++){ 
						  	$img=$formationC->display_img($row[$i]['id_formation']); ?>
						<div class="category-item list clear-fix">
							<div class="picture">
								<div class="hover-effect"></div>
								<div class="link-cont">
									<a href="display_course.php?id_formation=<?=$row[$i]['id_formation']?>" class="fancy fa fa-search"></a>
								</div>
								<img src="<?=$img['img']?>" data-at2x="<?=$img['img']?>" alt>
							</div>
							<h3>Course of <?=$row[$i]['nom'] ?></h3>
							<div>
								<div class="star-rating" title="Rated 4.00 out of 5">
									<span style="width:<?=($row[$i]['taux']*20)?>%"></span>
								</div>
							</div>
							<p><?=$row[$i]['description']?> </br> <?=$row[$i]['categorie']?> </p>
							<div class="category-info">
								<span class="price">
									<span class="amount">
										<?=$row[$i]['prix']?><sup>.DT</sup>
									</span>
								</span>
								<div class="course-lector">
									<div class="lector-name">
										<h4>By <?=$row[$i]['editor']?></h4>
										<span>Teacher</span>
									</div>
								</div>
							</div>
