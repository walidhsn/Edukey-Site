<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		{
			//save to database
			$insert = $con->prepare("UPDATE `blog` SET `name`='$name',`description`='$description',`text`='$text',`photo`='$photo' WHERE `id`='$id'");
			$insert->execute();
...
<html>
<head>
	<title>Edukey - Update Blog</title>
	<meta charset="utf-8">
			...
			<div class="grid-col grid-col-9">
				...
				<main>
					...
					<form method="POST">
						...
						<input type="text" name="name" placeholder="put a name ..." value="<?=$blog['name']?>" required>
						<br>
						<label for="name"><b>Blog Picture :</b></label>
						<input type="text" name="photo" placeholder="put photo link .." value="<?=$blog['photo']?>">
						<br>
						<label for="description"><b>Description :</b></label>
						<textarea name="description" id="" cols="60" rows="4"><?=$blog['description']?></textarea>
						<br>
						<label for="text"><b>Text :</b></label>
						<textarea name="text" id="" cols="140" rows="12" ><?=$blog['text']?></textarea>
						<br>
						<button type="submit" class="cws-button bt-color-3 border-radius alt icon-right">Update<i class="fa fa-angle-right"></i></button>
					</form>
					</aside>
				</main>
				<!-- / main content -->
