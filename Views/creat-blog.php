<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($name) && !empty($description) && !empty($creator))
		{
			//save to database
			$date_c = date("Y-m-d");
			$insert = $con->prepare("insert into blog (name,photo,description,text,creator,date) values ('$name','$photo','$description','$text','$creator','$date_c')");
			$insert->execute();
			header("Location:blog-default.php");
			die;
		}else
		{
			echo '<script>alert("Login in order to write names");</script>';
		}
	}
	//Partie : 
		...
		<div class="page-title">
			<div class="grid-row">
				...
				<nav class="bread-crumb">
					...
					<i class="fa fa-long-arrow-right"></i>
					<a href="creat-blog.php">Create Blog</a>
				</nav>
				<!-- / bread crumb -->
			</div>
		</div>
		...
		<div class="page-content grid-col-row clear-fix">
			<div class="grid-col grid-col-9">
				...
				<main>
					...
					<form method="POST">
						<br>
						<label for="name"><b>Blog Name :</b> </label>
						<input type="text" name="name" placeholder="put a name ..." required>
						<br>
						<label for="name"><b>Blog Picture :</b> </label>
						<input type="text" name="photo" placeholder="put photo link ..">
						<br>
						<label for="description"><b>Description :</b></label>
						<textarea name="description" id="" cols="60" rows="4" placeholder="Add Description"></textarea>
						<br>
						<label for="text"><b> Text :</b></label>
						<textarea name="text" id="" cols="140" rows="12" placeholder="Add Text" ></textarea>
						<br>
						<input type="text" name="creator" value="<?=$_SESSION['name']?>" hidden style="width:0px;height:0px;">
						<button type="submit" class="cws-button bt-color-3 border-radius alt icon-right">Publish<i class="fa fa-angle-right"></i></button>
					</form>
					</aside>
				</main>
				<!-- / main content -->
